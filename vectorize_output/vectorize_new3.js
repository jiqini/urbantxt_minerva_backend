...
/*
 * Vectorize Victim/Gender Violence Law PDF with LLM-Based Auto-Tagging for RAG
 * Usage: node vectorize_output/vectorize_new3.js
...
const Tags = [
  "victim protection", "gender violence", "domestic violence", "protective measures", "sexual violence",
  ...
  "court proceedings", "law enforcement", "criminal justice process", "human rights", "due process"
];

const data = require('../output_parsed_pdf/output_new3.json');
...
async function getTagsLLM(text) {
    const prompt = `Given the following victim/gender violence law text, select all applicable tags from this list ONLY (do not invent new tags, do not use synonyms): ${JSON.stringify(Tags)}. Avoid generic tags like "law" or "article".\n\nText: ${text}\n\nReturn the tags as a JSON array of lowercase strings, like:\n["victim protection", "gender violence", "due process"]`;
    for (let attempt = 0; attempt < 3; attempt++) {
        try {
            const response = await openai.chat.completions.create({
            model: 'gpt-3.5-turbo',
            messages: [
                { role: 'system', content: 'You are a victim/gender violence law expert for El Salvador.' },
...
async function storeVictimLawChunks(data) {
    ...
    for (const element of data) {
        ...
        for (const chunkText of chunks) {
            ...
            if (exists) {
                // Only print the skip message, not the chunk content
