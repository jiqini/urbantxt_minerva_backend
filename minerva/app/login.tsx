...
export default function LoginScreen() {
  ...
  try {
    ...
    const res = await fetch(`${process.env.EXPO_PUBLIC_SERVER_URL}/api/login`, {
      method: 'POST',
    ...
    if (res.ok) {
      console.log('✅ Login successful, saving user securely...');
      
      // Handle different response formats safely
      const userData = {
        id: data.user?.id || data.id || data.userId || username.trim(),
        username: data.user?.username || data.username || username.trim(),
        emailOrPhone: data.user?.emailOrPhone || data.email || data.emailOrPhone || ''
      };
      
      const token = data.token || data.accessToken || 'default-token';
