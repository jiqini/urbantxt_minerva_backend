...
export class LegalDocumentService {
  ...
  async populateDatabase(): Promise<void> {
    ...
    for (const doc of legalDocuments) {
      // Assuming addDocument is the correct method to add documents to Realm
      await databaseService.addDocument(doc); // Changed from mongoService
    }
  }

  async getLegalDocuments(): Promise<LegalDocumentSchema[]> {
    return await databaseService.getAllDocuments(); // Changed from mongoService
  }
