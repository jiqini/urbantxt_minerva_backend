...
export function AIProvider({ children }: AIProviderProps) {
  ...
  const generateDocument = async (type: string, details: any): Promise<AgentResponse | null> => {
    ...
    try {
      ...
      if (agent) {
        return await agent.process(`Generate a ${type} document with these details: ${JSON.stringify(details)}`);
      }
      return null;
    } catch (error) {
  ...
  const setDeadline = async (deadline: any): Promise<AgentResponse | null> => {
    ...
    try {
      ...
      if (agent) {
        return await agent.process(`Set deadline: ${JSON.stringify(deadline)}`);
      }
      return null;
    } catch (error) {
  ...
  const getScenarioAdvice = async (scenario: string): Promise<AgentResponse | null> => {
    ...
    try {
      ...
      if (agent) {
        return await agent.process(scenario);
      }
      return null;
    } catch (error) {
  ...
  const findResources = async (location: string, type: string): Promise<AgentResponse | null> => {
    ...
    try {
      ...
      if (agent) {
        return await agent.process(`Find ${type} resources in ${location}`);
      }
      return null;
    } catch (error) {
  ...
  const getEtiquetteAdvice = async (situation: string): Promise<AgentResponse | null> => {
    ...
    try {
      ...
      if (agent) {
        return await agent.process(situation);
      }
      return null;
    } catch (error) {
